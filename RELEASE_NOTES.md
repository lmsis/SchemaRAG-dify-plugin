...
| Item | Valor |
|------|--------|
| **Versão atual no repo** | `1.0.4` — plugin id **`lmsis/lm_db_schema_rag`** |
| **Próxima versão sugerida** | `1.0.5` ou `1.1.0` — conforme alterações |
...
4. **`UPDATE.md`** — histórico de versões (a partir de **`1.0.0`** como linha estável).
...
`v1.0.0 — LM DB Schema RAG: primeira release estável (lmsis/lm_db_schema_rag)`

### Tag Git (exemplo)

`v1.0.0` ou `1.0.0` — *tag alinhada ao `manifest.yaml`.*
...
- [ ] Ajustado o número da versão no **título** do GitHub Release se não for `v1.0.0`.
