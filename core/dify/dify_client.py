...
class KnowledgeBaseClient(DifyClient):
    ...
    def create_dataset(self, name: str, **kwargs):
        ...
        data = {
            ...
            "permission": kwargs.get("permission", "only_me"),
    ...
    def create_document_by_file(
        ...
        logger.info(
            f"Opening file for upload: {file_path} (size: {os.path.getsize(file_path)} bytes)"
        )

        with open(file_path, "rb") as file_obj:
            files = {"file": (os.path.basename(file_path), file_obj, None)}
            data = {
                "process_rule": {"mode": "automatic"},
                "indexing_technique": "high_quality",
            }
    ...
    def update_document_by_file(
        ...
        logger.info(
            f"Opening file for update: {file_path} (size: {os.path.getsize(file_path)} bytes)"
        )
