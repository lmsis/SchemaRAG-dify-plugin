...
- Criaste uma release com tag (ex.: `v0.2.2`) mas **sem** subir um `.difypkg`.
...
2. Gera `{name}-{version}.difypkg` (ex.: `lm_db_schema_rag-0.2.2.difypkg`) com o CLI oficial.
3. Faz **upload** desse ficheiro para a mesma release.

**Passos:**

1. Faz **commit** e **push** deste workflow para `main`.
2. Na release **já existente** `v0.2.2`, podes:
...
2. Na raiz do repositório do plugin:
   ...
   /caminho/para/dify-plugin-darwin-arm64 plugin package . -o lm_db_schema_rag-0.2.2.difypkg
   ```

   Ajusta o nome do ficheiro de saída a `lm_db_schema_rag-<versão do manifest>.difypkg`.

3. No GitHub: **Releases** → abre a release **v0.2.2** → **Edit release** → arrasta o `.difypkg` para **Attach binaries** → guarda.

4. No Dify, volta a **Install from GitHub** e escolhe a versão; o pacote deve aparecer no dropdown.

## Nome do pacote no Dify

O instalador lista todos os `.difypkg` da release. O nome típico é **`lm_db_schema_rag-0.2.2.difypkg`** (campo `name` + `version` do `manifest.yaml`). Se no teu ecrã aparecer outro padrão, é só o rótulo da UI; o importante é a extensão **`.difypkg`**.
