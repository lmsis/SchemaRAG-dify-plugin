...
- Criaste uma release com tag (ex.: `v0.1.7`) mas **sem** subir um `.difypkg`.
- O workflow `plugin-publish.yml` deste repo empacota o plugin para o ecossistema `dify-plugins`, **não** anexa o pacote à tua release.
...
2. Gera `{name}-{version}.difypkg` (ex.: `schemarag-0.1.7.difypkg`) com o CLI oficial.
3. Faz **upload** desse ficheiro para a mesma release.

**Passos:**

1. Faz **commit** e **push** deste workflow para `main`.
2. Na release **já existente** `v0.1.7`, podes:
...
2. Na raiz do repositório do plugin:
   ...
   /caminho/para/dify-plugin-darwin-arm64 plugin package . -o schemarag-0.1.7.difypkg
   ```

   Ajusta o nome do ficheiro de saída a `schemarag-<versão do manifest>.difypkg`.

3. No GitHub: **Releases** → abre a release **v0.1.7** → **Edit release** → arrasta o `.difypkg` para **Attach binaries** → guarda.

4. No Dify, volta a **Install from GitHub** e escolhe a versão; o pacote deve aparecer no dropdown.

## Nome do pacote no Dify

O instalador lista todos os `.difypkg` da release. O nome típico é **`schemarag-0.1.7.difypkg`** (nome + versão do `manifest.yaml`). Se no teu ecrã aparecer outro padrão, é só o rótulo da UI; o importante é a extensão **`.difypkg`**.
